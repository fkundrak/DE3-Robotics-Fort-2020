...
Phase 2 - Building A Fort with Baxter Using One Arm
...
def main():
    ...
    def left(ldata):
        ...
        if len(ldata)==8:
            print "\nrationant "
            lplace_pose = poseratioant(ldata[2],ldata[3]-.01,ldata[4],ldata[5],ldata[6],ldata[7])
            # Creates the pose using the function 'poseratioant'
        else:
            print "\ndefined "
            lplace_pose = posedefined(ldata[2],ldata[3]-.01,ldata[4],ldata[5],ldata[6],ldata[7],ldata[8])
    ...
    def right(rdata):
        ...
        if len(rdata)==8:
            print "\nrationant "
            rplace_pose = poseratioant(rdata[2],rdata[3]-.01,rdata[4],rdata[5],rdata[6],rdata[7])
            # Creates the pose using the function 'poseratioant'
        else:
            print "\ndefined "
            rplace_pose = posedefined(rdata[2],rdata[3]-.01,rdata[4],rdata[5],rdata[6],rdata[7],rdata[8])
            # Creates the pose using the function 'posedefined'
